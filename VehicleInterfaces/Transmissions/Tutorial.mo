...
<p><strong>Tutorial - Defining a new manual transmission model</strong></p>
<p>The following process will demonstrate how to create a new manual transmission model using this interface definition.</p>
<ol>
<li>Create a new model that extends <strong>VehicleInterfaces.Transmissions.Interfaces.BaseManualTransmission</strong>, it should look like this:
...
<li>This dialog allows you to enable/disable the optional connections by setting <strong>includeClutchPedal</strong>, <strong>includeManualShiftConnector</strong> and <strong>includeMount</strong> as required for your new transmission model.  The <strong>engineFlange</strong> and <strong>drivelineFlange</strong> connectors are of the type <a href=\"modelica://Modelica.Mechanics.MultiBody.Interfaces.FlangeWithBearing\">Modelica.Mechanics.MultiBody.Interfaces.FlangeWithBearing</a>, the parameters <strong>includeEngineBearing</strong> and <strong>includeDrivelineBearing</strong> controls whether the bearing connectors within these connections are enabled or not.</li>
...
<li>Next, we need to add the required connections to the control signal bus for the transmission, see <a href=\"Modelica://VehicleInterfaces.UsersGuide.SignalBus\">here</a> for a complete list of the minimum connections required.  As we are creating a manual transmission model we need to add three signals to the transmissionBus which is part of the controlBus.  We need to put the transmission output speed, current gear and the clutch state on to the transmissionBus.  As this is a simple single gear transmission the current gear and clutch state can be set as constants.  Start by adding the transmissionBus connector which can be found at <strong>VehicleInterfaces.Interfaces.TransmissionBus</strong> and connect this to the controlBus.  When this connection is made the following dialog will be produced and should be completed as shown.
<br><br><img src=\"modelica://VehicleInterfaces/Resources/Images/Transmissions/buildNewTransmission4.png\"><br><br></li>
<li>We can now connect the constants for clutch state and current gear to the transmissionBus.  The transmission output speed needs to be in the correct units, in this case r/min (or rpm).  Add a rotational speed sensor and a unit conversion block from <strong>Modelica.Blocks.Math.UnitConversions</strong>, set the conversion block to convert to rpm.  When you create the connection between the transmissionBus and one of these blocks a dialog like the one below will be produced.  You will need to complete the dialog using one of the following names:
<ul>
<li>gear</li>
<li>clutchLocked</li>
<li>outputSpeed</li>
