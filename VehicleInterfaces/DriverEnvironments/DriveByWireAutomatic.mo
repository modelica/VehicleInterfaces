...
public
  ...
  Modelica.Blocks.Sources.Step brakeDemand(
    ...
    startTime=brakeTime) annotation (Placement(transformation(extent={{-60,50},{-40,70}})));
  Modelica.Blocks.Sources.Step acceleratorDemand(
    height=finalAccelRequest,
    offset=initialAccelRequest,
    startTime=accelTime) annotation (Placement(transformation(extent={{-20,70},{0,90}})));
...
equation
  ...
  connect(gearMode.y, driverBus.gearboxMode) annotation (Line(points={{-39,20},{62,20},{62,60.05},{60.05,60.05}},
        color={255,127,0}));
  connect(ignition.y, driverBus.ignition) annotation (Line(points={{1,40},{60,40},{60,60}}, color={255,127,0}));
