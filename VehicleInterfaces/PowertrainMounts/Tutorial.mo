...
<p><strong>Tutorial - Defining a new powertrain mounting system</strong></p>
...
<ol>
<li>We are creating a powertrain mounting system to support 2 powertrain systems so we need to base our model on the <strong>DualSystemMount</strong> Interface definition.  Start by extending this class <strong>VehicleInterfaces.PowertrainMounts.Interfaces.DualSystemMount</strong></li>
<li>This interface definition contains 3 MultiBody connectors, the one called <strong>chassisFrame</strong> will be attached to the chassis subsystem in our vehicle model and is the chassis reference frame.  The other two connectors, <strong>mount_1</strong> and <strong>mount_2</strong> are the reference frames for the two subsystems that this mounting system will support.  In this case <strong>mount_1</strong> will be treated as the engine reference frame and <strong>mount_2</strong> as the transmission reference frame.  The first step is to add a fixed translation (<strong>Modelica.Mechanics.MultiBody.Parts.FixedTranslation</strong>) between mount_1 and mount_2 to define the position of the transmisison reference frame relative to the engine reference frame.
...
<li>Add the elastic mount blocks, in this case they are defined in the PowerTrain library as <strong>PowerTrain.MountingSystems.Mount</strong>
